...
engine/
  ...
  coding-opportunities.mjs ← evidence-bound CPT/HCPCS and ICD-10-CM review
                       candidates with payer/documentation caveats.
  output-work.mjs   ← persistent tasks and editable generated document drafts.
...
npm test                          # complete engine, workflow, auth, and cloud tests
```
...
The 24-hour workflow is active as an Amaris-only pilot. It starts
...
Freed clipboard intake, and raises 12/20/24-hour browser alerts. Documentation
intelligence now creates evidence-bound coding opportunities that must be
applied or dismissed by the provider; it never infers a diagnosis from a
medication, symptom, or test result. Detected downstream work becomes a
completable task plus an editable, synchronized draft that can be downloaded
when CharmHealth has no matching destination. The approved
...
The protected Google Cloud queue adds cross-device persistence through Cloud
Run and the named Firestore database. Clinical packets travel directly between
the signed-in browser and Google Cloud; Netlify serves the UI and issues a
short-lived allowlisted token but does not receive the packet content. See
`docs/GOOGLE_CLOUD_QUEUE.md` for deployment and access controls.

The browser-only mode remains available until the Cloud Run URL and shared
token secret are configured. Test the cloud path with synthetic data before
