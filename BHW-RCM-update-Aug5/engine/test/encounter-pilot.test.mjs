...
test("persistent pilot queue excludes note text and clinical codes", () => {
  ...
  const hydrated = parseQueue(stored, { "ENC-1": { note: "Session-only note", codes: ["99214"], diagnoses: ["I10"], tasks: [{ id: "task:instructions", type: "instructions", title: "Patient instructions", status: "complete" }] } });
  ...
  assert.equal(hydrated[0].tasks.find((task) => task.id === "task:instructions").status, "complete");
});
