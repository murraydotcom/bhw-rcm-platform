...
test("loadUsers keeps hash-less entries (Google-only) with their role", () => {
  const prev = process.env.AUTH_USERS;
  process.env.AUTH_USERS = JSON.stringify([
    { email: "approved@bhwmedical.org", name: "Approved User", role: "admin" }, // no hash
  ]);
  try {
    const u = auth.findUser("approved@bhwmedical.org");
...
test("requireAuth fails closed when AUTH_SECRET is unset", () => {
