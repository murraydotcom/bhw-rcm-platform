...
function mapRow(db, p) {
  if (db === "claims") {
    ...
    return {
      ...
      check:        first(p, ["Check/EFT No", "Check/EFT Number", "Check/EFT #", "Check Number", "Check/Ref"]),
  ...
  if (db === "payments") {
    // Manual EOB / cash / check postings recorded by payment-post.js.
    return {
      patient: first(p, ["Patient", "Patient Name"]),
      payer:   stripId(first(p, ["Payer", "Payer Name"])),
      type:    first(p, ["Type"]),
      method:  first(p, ["Method"]),
      check:   first(p, ["Check/Ref", "Check/EFT No", "Check Number", "Check/EFT #"]),
      claim:   first(p, ["Claim #", "Claim Number", "Patient Ctl No", "Invoice #"]),
      date:    normDate(first(p, ["Date", "Payment Date"])),
      ...
      reason:  first(p, ["Reason", "Remit Remarks"]),
    };
  }
