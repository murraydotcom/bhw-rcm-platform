...
const SAVED = {};
before(() => {
  for (const k of ["AUTH_SECRET", "GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "GOOGLE_ALLOWED_DOMAIN", "GOOGLE_REDIRECT_URI"]) SAVED[k] = process.env[k];
...
test("verifyClaims accepts a valid Workspace identity (hd match)", () => {
  ...
  assert.equal(id.email, "amaris@bhwmedical.org");
  assert.equal(id.name, "Amaris Murray");
});

test("verifyClaims accepts domain via email when hd absent", () => {
  const id = g.verifyClaims(g.decodeJwt(jwt(validClaims({ hd: undefined }))));
  assert.equal(id.email, "amaris@bhwmedical.org");
});
